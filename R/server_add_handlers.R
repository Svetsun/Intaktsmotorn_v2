...
register_add_handlers <- function(input, session, rv,
  ...
  observeEvent(input$btn_add_uppgift, {
    ...
    errs <- validate_add_form(list(
      list(
        ok = { s <- safe_as_date(input$uppg_start); e <- safe_as_date(input$uppg_slut)
               is.na(s) || is.na(e) || s <= e },
        msg = "Startdatum m\u00e5ste vara f\u00f6re eller lika med slutdatum."
      ),
      list(
    ...
    df <- sanitize_nulls_df(rv$wb[["Uppgift"]]) |>
      ...
      ensure_cols(c("slutdatum"), default = NA_character_) |>
    ...
    new_row <- data.frame(
      ...
      slutdatum = safe_as_date(input$uppg_slut),
    ...
    reset_form(session, ids = c("uppg_kund","uppg_upp","uppg_name","uppg_kons","uppg_timpris","uppg_start","uppg_slut"))
